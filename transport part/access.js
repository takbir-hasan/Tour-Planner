$(document).ready(function () {
  const apiUrl = 'http://localhost:3000/api/transports'; // Corrected API URL
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            const filteredTransports = response.filter(transport => {
                return (
                    transport.location.toLowerCase() === destination.toLowerCase() &&
                    transport.date.includes(date) && transport.maxPassengers >= passengers
                );
            });

            // Sort filtered guides by rating
            const sortedTransports = filteredTransports.sort((a, b) => b.rating - a.rating);

            // Display filtered and sorted guides
            displayResults(sortedTransports);
        },
        error: function (error) {
  ...
  $('#search-btn').on('click', function () {
    ...
    const passengers = parseInt($('#passengers').val(), 10);
